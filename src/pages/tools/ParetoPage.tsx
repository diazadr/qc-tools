...
const ParetoPage = () => {
  ...
  const [showABCLine, setShowABCLine] = useState(false);
  const [showABCLabel, setShowABCLabel] = useState(false);
...
<div className="p-3 border border-border rounded bg-card shadow-sm space-y-4">
    ...
    <div className="space-y-3">
      ...
      <label className="flex flex-col text-xs">
        ...
        <select
          ...
          onChange={(e) => l.setNormalizationMode(e.target.value as NormalizationMode)}
          className="h-[36px] bg-bg border border-border rounded px-2 mt-1"
        >
  ...
  <div className="flex flex-wrap items-center gap-4 pt-2">
    ...
    <label className="flex items-center gap-2 text-xs">
      <input
        type="checkbox"
        checked={showABCLine}
        onChange={() => setShowABCLine(v => !v)}
      />
      Show ABC Lines (80% + 95%)
    </label>

    <label className="flex items-center gap-2 text-xs">
      <input
        type="checkbox"
        checked={showABCLabel}
        onChange={() => setShowABCLabel(v => !v)}
      />
      Show ABC Labels
    </label>
  ...
  <div className="flex justify-end pt-2">
    <button
      onClick={l.clearAll}
      className="h-[32px] px-3 bg-error/60 text-white rounded border-[0.5px] hover:border-error"
    >
      Hapus semua data
    </button>
  </div>
      ...
      <div className="qc-card mb-6">
        ...
        <ParetoChart
          ...
          showABCLine={showABCLine}
          showABCLabel={showABCLabel}
          yLeftLabel="Count"

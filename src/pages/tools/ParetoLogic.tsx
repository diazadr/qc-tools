...
useEffect(() => {
  try {
    ...
    if (encoded) {
      ...
      if (typeof decoded.customNormalizationBase === "number")
        setCustomNormalizationBase(decoded.customNormalizationBase);
      if (typeof decoded.smallGroupThreshold === "number")
        setSmallGroupThreshold(decoded.smallGroupThreshold);
      if (decoded.selectedSource) setSelectedSource(decoded.selectedSource);
      if (decoded.metadata) setMetadata(decoded.metadata);

      setLoadedFromLink(true); // penting
      return;
    }
  } catch {}
  
  // jika tidak ada link → pakai snapshot store
  ...
  if (snap?.data) {
    ...
    if (data.customNormalizationBase) setCustomNormalizationBase(data.customNormalizationBase);
    if (data.smallGroupThreshold) setSmallGroupThreshold(data.smallGroupThreshold);
...
useEffect(() => {
  ...
  const tableRows: ParetoRow[] = useMemo(() => {
    ...
    return sorted.map(it => {
      ...
      if (normalizationMode !== "none" && totalCount > 0) {
        ...
        else if (normalizationMode === "custom")
          base = customNormalizationBase > 0 ? customNormalizationBase : 1000
 ...
 const exportPDF = () => {
  ...
  doc.setFontSize(15);
  doc.text("Pareto Report", 14, 14);
  doc.setTextColor(0, 0, 0);

  // WATERMARK
  doc.setFontSize(48);
  doc.setTextColor(235);
  doc.text("PARETO", 40, 180, { angle: 30 });
  doc.setTextColor(0, 0, 0);
  ...
  doc.setFontSize(12);
  doc.text("Detail Analysis:", 14, fy);
  fy += 6;
  doc.setFontSize(10);
  ...
  const exportExcel = async () => {
    ...
    const sheet = workbook.addWorksheet("Pareto Data");

    sheet.addRow(["QC — Pareto Report"]);
    ...
    sheet.addRow(["Kategori", "Jumlah", "Nilai", "Cum%", "Tanggal"]);
    ...
    row = sheet.addRow(["Ringkasan"]);
    row.getCell(1).font = { bold: true };

    sheet.addRow([summary || "-"]);
    sheet.addRow([]);

    row = sheet.addRow(["Detail Analisis"]);
    row.getCell(1).font = { bold: true };

    sheet.addRow([summaryDetailText || "-"]);
    ...
    if (chartRef.current) {
      ...
      if (png) {
        chartImage = workbook.addImage({
          base64: png,
          extension: "png",
        });

        sheet.addImage(chartImage, {
          tl: { col: 6, row: 1 },   // top-left placement
